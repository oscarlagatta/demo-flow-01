...
import { getCategoryFromSection } from "./node-icon-mapping"
...
export function transformApiData(
  ...
  const transformedNodes: AppNode[] = apiData.nodes
    .map((apiNode): AppNode | null => {
      ...
      const sectionCategory = getCategoryFromSection(parentId)
      ...
      return {
        ...
        data: {
          ...
          icon: sectionCategory, // Assign section category as icon identifier
        },
        parentId: parentId,
